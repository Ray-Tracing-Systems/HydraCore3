...
std::pair<SpecN, FourierSpec> Integrator::CVGetSpectrum(uint specId, const SpecN &wavelengths, bool bounded)
{
    if(specId == 0xFFFFFFFF) return {SpecN(0.0f), FourierSpec(0.0f)};

    if(m_mese_cache.size() == 0) {
      m_mese_cache.resize(m_spec_values.size());
      std::fill_n(m_mese_cache.begin(), m_spec_values.size(), std::numeric_limits<float>::infinity());
    }
    ...
    FourierSpec specf = FourierSpec(m_spec_values + offset, size);

    std::vector<float> mese_q;
    if(bounded) {
      if(m_mese_cache[offset] == std::numeric_limits<float>::infinity()) {
        mese_q = lagrange_multipliers(specf);
        std::copy_n(mese_q.begin(), FourierSpec::SIZE, m_mese_cache.data() + offset);
      }
      else {
        mese_q.resize(FourierSpec::SIZE);
        std::copy_n(m_mese_cache.data() + offset, FourierSpec::SIZE, mese_q.begin());
      }

      SpecN specn;
      for(int i = 0; i < SpecN::SIZE; ++i) {
        specn[i] = bounded_mese_l(to_phase(wavelenghts[i]), mese_q);
      }
    }
    else {
      if(m_mese_cache[offset] == std::numeric_limits<float>::infinity()) {
        mese_q = precompute_mese_coeffs(specf);
        std::copy_n(mese_q.begin(), FourierSpec::SIZE, m_mese_cache.data() + offset);
      }
      else {
        mese_q.resize(FourierSpec::SIZE);
      ...
      SpecN specn;
      for(int i = 0; i < SpecN::SIZE; ++i) {
        specn[i] = mese_precomp(to_phase(wavelenghts[i]), mese_q);
      }
    }
    
    return std::make_pair(std::move(specn), std::move(specf));
}


void Integrator::PathTraceCV(uint tid, uint channels, float* out_color)
{
  SpecN accumColor, accumThroughput;
  FourierSpec accumColor_f, accumThroughput_f;
  ...
  uint      rayFlags;
  float     time;
  kernel_InitEyeRay2CV(tid, &rayPosAndNear, &rayDirAndFar, &wavelengths, &accumColor, &accumColor_f, &accumThroughput, &accumThroughput_f, &gen, &rayFlags, &mis, &time);

  for(uint depth = 0; depth < m_traceDepth; depth++) 
  {
    SpecN shadeColor;
    FourierSpec shadeColor_f;
    ...
    kernel_SampleLightSourceCV(tid, &rayPosAndNear, &rayDirAndFar, &wavelengths, &hitPart1, &hitPart2, &hitPart3, &rayFlags, &time,
...
void Integrator::kernel_InitEyeRay2CV(uint tid, float4* rayPosAndNear, float4* rayDirAndFar, SpecN* wavelengths, 
                                    SpecN* accumColor, FourierSpec* accumColor_f, 
                                    SpecN* accumuThoroughput, FourierSpec accumuThoroughput_f,
                                    RandomGen* gen, uint* rayFlags, MisData* misData, float* time) // 
{
  ...
  *accumColor_f      = FourierSpec(0.0f);
  *accumThroughput_f = FourierSpec(1.0f);
...
void Integrator::kernel_SampleLightSourceCV(uint tid, const float4* rayPosAndNear, const float4* rayDirAndFar, const SpecN* wavelengths,
                                          ...
                                          SpecN* out_shadeColor, FourierSpec *out_shadeColor_f)
{
  if(tid >= m_maxThreadId)
    return;
  ...
  if(lightId < 0) // no lights or invalid light id
  {
    ...
    *out_shadeColor_f = FourierSpec(0.0f);
    return;
  }
  ...
  {
    const BsdfEvalCV bsdfV  = MaterialEvalCV(matId, &lambda, shadowRayDir, (-1.0f)*ray_dir, hit.norm, hit.tang, hit.uv);
    ...
    const auto [lightColor, lightColor_f] = LightIntensityCV(lightId, lambda, shadowRayPos, shadowRayDir);

    const float t = cosThetaOut * misWeight / lgtPdf;

    *out_shadeColor   = (lightColor * bsdfV.val) * t;
    *out_shadeColor_f = (lightColor_f * bsdfV.val_f) * t; 
...
std::pair<SpecN, FourierSpec> Integrator::LightIntensityCV(uint a_lightId, const SpecN &a_wavelengths, float3 a_rayPos, float3 a_rayDir)
{  
  ...
  const uint offset = data.x;
  const uint size   = data.y;
  auto lightColor = CVGetSpectrum(m_spec_values.data() + offset, a_wavelengths);

  lightColor->first  *= m_lights[a_lightId].mult;
  lightColor->second *= m_lights[a_lightId].mult;
