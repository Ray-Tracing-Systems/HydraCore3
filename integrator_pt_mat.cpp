                                        ...
                                        uint tid, MisData* a_misPrev, BsdfSample* a_pRes)
{
  ...
  const float select = GetRandomNumbersMatB(tid, a_gen, bounce, layer);
                                             ...
                                             MisData* a_misPrev, const uint a_currRayFlags, uint tid)
{
  ...
  {
    currMatId = BlendSampleAndEval(currMatId, bounce, layer, wavelengths, a_gen, v, n, tc, tid, a_misPrev, &res);
  ...
  const float4 rands     = GetRandomNumbersMats(tid, a_gen, bounce);
