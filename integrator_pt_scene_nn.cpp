...
Material LoadNeuralBrdfMaterial(const pugi::xml_node& materialNode, const std::vector<TextureLoadInfo> &texturesInfo,
                                std::unordered_map<HydraSampler, uint32_t, HydraSamplerHash> &texCache,
                                std::vector<std::shared_ptr<ICombinedImageSampler>> &textures,
                                ...
                                std::vector<float> &m_neural_weights, std::vector<uint> &m_neural_weights_offsets)
{
  ...
  m_neural_tex_offsets[id] = {uint(m_neural_tex_ids.size()), uint(loaded_tex.size())};
