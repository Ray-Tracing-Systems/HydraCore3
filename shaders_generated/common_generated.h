...
struct LightSource
{
  ...
  //vec4 dummy;     ///<! don't forget to manually align stucture to 16 bytes when add new fields
};
struct LightSample
{
  vec3 pos;
...
vec3 hydraFresnelCond(vec3 f0, float VdotH, float ior, float roughness) {  
  if(ior == 0.0f) // fresnel reflactance is disabled
    return f0;

  return f0 + (vec3(1.0f,1.0f,1.0f) - f0) * fresnelSlick(VdotH); // return bsdf * (f0 + (1 - f0) * (1 - abs(VdotH))^5)
}
