...
static void gltfEval(const Material* a_materials, float3 l, float3 v, float3 n, float2 tc, float4 color,
                     BsdfEval* res)
{
  ...
  if((cflags & GLTF_COMPONENT_COAT) != 0 && (cflags & GLTF_COMPONENT_LAMBERT) != 0) // Plastic, account for retroreflection between surface and coating layer
  {
    const float vDotN = std::abs(dot(v,n));
    const float lDotN = std::abs(dot(l,n));
    f_i                                    = FrDielectricPBRT(vDotN, 1.0f, fresnelIOR);
    //const float f_o                        = FrDielectricPBRT(lDotN, 1.0f, fresnelIOR);  
    //const float m_fdr_int                  = a_materials[0].data[GLTF_FLOAT_MI_FDR_INT];
