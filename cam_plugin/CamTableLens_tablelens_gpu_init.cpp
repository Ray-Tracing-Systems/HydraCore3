...
VkBufferUsageFlags CamTableLens_TABLELENS_GPU::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t CamTableLens_TABLELENS_GPU::GetDefaultMaxTextures() const { return 256; }

void CamTableLens_TABLELENS_GPU::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void CamTableLens_TABLELENS_GPU::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
CamTableLens_TABLELENS_GPU::~CamTableLens_TABLELENS_GPU()
{
  ...
  vkDestroyDescriptorSetLayout(device, MakeEyeRayDSLayout, nullptr);
  MakeEyeRayDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorSetLayout(device, ContribSampleDSLayout, nullptr);
  ContribSampleDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorPool(device, m_dsPool, NULL); m_dsPool = VK_NULL_HANDLE;

 
  vkDestroyBuffer(device, m_classDataBuffer, nullptr);

  vkDestroyBuffer(device, m_vdata.linesBuffer, nullptr);
  ...
  vkDestroyBuffer(device, m_vdata.m_storedCos4Buffer, nullptr);
  vkDestroyBuffer(device, m_vdata.m_storedWavesBuffer, nullptr);
  FreeAllAllocations(m_allMems);
}

void CamTableLens_TABLELENS_GPU::InitHelpers()
{
  vkGetPhysicalDeviceProperties(physicalDevice, &m_devProps);
}


void CamTableLens_TABLELENS_GPU::InitKernel_MakeEyeRay(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_tablelens_gpu/kernel1D_MakeEyeRay.comp.spv"); 
  const VkSpecializationInfo* kspec = nullptr;
  MakeEyeRayDSLayout = CreateMakeEyeRayDSLayout();
  MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, MakeEyeRayDSLayout, &MakeEyeRayLayout, &MakeEyeRayPipeline);
}

void CamTableLens_TABLELENS_GPU::InitKernel_ContribSample(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_tablelens_gpu/kernel1D_ContribSample.comp.spv"); 
  const VkSpecializationInfo* kspec = nullptr;
  ContribSampleDSLayout = CreateContribSampleDSLayout();
  MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, ContribSampleDSLayout, &ContribSampleLayout, &ContribSamplePipeline);
}


void CamTableLens_TABLELENS_GPU::InitKernels(const char* a_filePath)
{
  InitKernel_MakeEyeRay(a_filePath);
  InitKernel_ContribSample(a_filePath);
}
...
void CamTableLens_TABLELENS_GPU::InitMemberBuffers()
{
  ...
  m_vdata.linesBuffer = vk_utils::createBuffer(device, lines.capacity()*sizeof(struct LensElementInterface), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.linesBuffer);
...
void CamTableLens_TABLELENS_GPU::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[CamTableLens_TABLELENS_GPU::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
CamTableLens_TABLELENS_GPU::MemLoc CamTableLens_TABLELENS_GPU::AllocAndBind(const std::vector<VkBuffer>& a_buffers)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
CamTableLens_TABLELENS_GPU::MemLoc CamTableLens_TABLELENS_GPU::AllocAndBind(const std::vector<VkImage>& a_images)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "CamTableLens_TABLELENS_GPU::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    } 
...
void CamTableLens_TABLELENS_GPU::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void CamTableLens_TABLELENS_GPU::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 CamTableLens_TABLELENS_GPU::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
CamTableLens_TABLELENS_GPU::MegaKernelIsEnabled CamTableLens_TABLELENS_GPU::m_megaKernelFlags;
