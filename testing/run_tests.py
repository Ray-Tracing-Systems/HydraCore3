...
class REQ_H2X(REQ):
  ...
  def test(req, gpu_id=0):
    for (scnp, imgp, id) in zip(req.scn_path, req.ref_path, range(0,len(req.scn_path))):
  ...
  def test(req, gpu_id=0):
    for (scnp, imgp, id) in zip(req.scn_path, req.ref_path, range(0,len(req.scn_path))):
      ...
      for dev_type in devices:
        Log().info("  rendering scene: '{0}', dev_type='{1}', dev_id = '{2}'".format(test_name, dev_type, gpu_id))
        for inregrator in req.integs:
          outp = PATH_TO_HYDRA3_SCENS + "/Report/Images/" + test_name  + "/y" + str(id) + "_" + dev_type + inregrator + ".bmp"
          args = ["./cmake-build-release/hydra", "-in", scene_path, "-out", outp, "-integrator", inregrator, "-spp-naive-mul", str(req.naivem), "-gamma", "2.2"]
          args = args + ["-gpu_id", str(gpu_id)]  # for single launch samples
          args = args + ["-width", str(req.imsize[0]), "-height", str(req.imsize[1])]
          args = args + ["--" + dev_type]
...
reqs.append( REQ_H2X("mat_emission", [PATH_TO_HYDRA2_TESTS + "/tests_f/test_123/statex_00001.xml",  
                                     ...
                                     [PATH_TO_HYDRA2_TESTS + "/tests_images/test_123/w_ref.png", 
                                      ...
                                      PATH_TO_HYDRA2_TESTS + "/tests_images/test_123/w_ref3.png"], naivemul = 16, imsize = (512,512), inregrators = ["naivept","mispt"]))
